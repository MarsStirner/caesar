...
from ..info.EventInfo import CServiceInfo
from ..info.PrintInfo import CInfo, CInfoList, CDateInfo, CRBInfo
...
class CAccountItemInfo(CInfo):
    ...
    def _load(self):
        from ..info.EventInfo import CEventInfo, CVisitInfo
        ...
        sum_conv = NumToTextConverter(forceDouble(record.value('sum')))
        self._sumInWords  = sum_conv.convert().getRubText() + sum_conv.convert().getKopText()
        #self._nds         = forceDouble(record.value('nds'))
        #self._ndsInWords  = amountToWords(forceDouble(record.value('nds')))
        self._date        = CDateInfo(forceDate(record.value('date')))
        self._number      = forceString(record.value('number'))
        self._refuseType  = self.getInstance(CRBPayRefuseTypeInfo, forceRef(record.value('unit_id')))
    ...
    sumInWords = property(lambda  self: self.load()._sumInWords)
    #nds = property(lambda self: self.load()._nds)
    #ndsInWords = property(lambda self: self.load()._ndsInWords)
...
class CRBAccountFormatInfo(CRBInfo):
    ...
    def _initByRecord(self, record):
        self._prog = forceBool(record.value('prog'))
...
class CAccountInfo(CInfo):
    ...
    def _load(self):
        from library.printing.info.EventInfo import CContractInfo
        ...
        self._org = self.getInstance(COrgInfo, forceRef(record.value('org')))
        return result
